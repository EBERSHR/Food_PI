...
async function getRecipes(req, res) {
    ...
    let menuDB = [];
    let itemMenu = {};
    // await axios.get('https://api.spoonacular.com/recipes/complexSearch?apiKey=e592a252bef543818a79100843fa31fb&addRecipeInformation=true')
    //     .then(response => {
    // const data = response.data.results;

    const data = dataRecipes.results;

    data.map(element => {
        itemMenu.id = element.id
        itemMenu.name = element.title;
        itemMenu.image = element.image;
        itemMenu.score = element.spoonacularScore;
        itemMenu.level = element.healthScore;
        itemMenu.resume = element.summary;
        // itemMenu.steps = element.analyzedInstructions[0].steps;


        itemMenu.types = element.diets;
        menuApi.push(itemMenu);
        itemMenu = {};
    ...
    Recipe.findAll({ include: { model: Type } })
        .then(response => {
            response.map(element => {
                let tipos = [];
                itemMenu.id = element.id
                itemMenu.name = element.name;
                itemMenu.image = element.image;
                itemMenu.score = element.score;
                itemMenu.level = element.level;
                itemMenu.resume = element.resume;
                itemMenu.steps = element.steps;
                element.types.map(tipo => {
                    tipos.push(tipo.name)
                });
                itemMenu.types = tipos;
                menuDB.push(itemMenu);
                itemMenu = {};
            });
            res.send([...menuDB, ...menuApi])
        })
}

async function addRecipes(req, res) {
    const { nivel, nombre, pasos, puntuacion,
        resumen, image, types } = req.body;
    ...
    await nuevaReceta.addTypes(types);
...
async function getRecipeId(req, res) {
    ...
    try {
        await axios.get(`https://api.spoonacular.com/recipes/${id}/information?apiKey=e592a252bef543818a79100843fa31fb`)
        .then(response => {
            const { id, title, image, spoonacularScore, healthScore, summary, analyzedInstructions, diets } = response.data;
            const recipeId = {
                id: id,
                name: title,
                image: image,
        ...
        console.log('Buscando en la base de datos.....', id)
        ...
        .then((response) => {
            // const { id, name, image, score, level, resume, steps, types } = response;
            // const recipeId = {
            //     id: response.id,
            //     name: response.name,
